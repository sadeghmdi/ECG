...
class DataHandling:
    ...
    def make_dataset(self, records=None):
        ...
        beat_feats = [[i for i in beat_feat.values()]
                      for beat_feat in beat_feats]
    ...
    def beat_info_feat(self, data, beat_loc=10):
        ...
        assert len(data['waveforms']) == len(
            data['start_idxs']), "len must be equal!"
        ...
        for i in tqdm(range(len(data['rpeaks']))):
            ...
            ten_segs, aggs = beatinfo.sub_segs(
                n_subsegs=10)  # many features a lis of lists
    ...
    def save_dataset_single(self, record, clean=True, split_ratio=0.3, save_file_name=None):
        ...
        ds_train = {'waveforms': xarr_train,
                    'beat_feats': farr_train, 'labels': yarr_train}
        ds_test = {'waveforms': xarr_test,
                   'beat_feats': farr_test, 'labels': yarr_test}
    ...
    def save_dataset_intra(self, records, clean=True, split_ratio=0.3, save_file_prefix='intra'):
        ...
        ds_train = {'waveforms': xdata_train,
                    'beat_feats': fdata_train, 'labels': ydata_train}
        ds_test = {'waveforms': xdata_test,
