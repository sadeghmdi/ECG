...
start_idx
	...
	def __call__(self,data):
		...
		self.whole_wavefrm = data['waveform']  #Whole waveform
		...
		self.absmeandrri = self.absmean_sdrri()
		self.bwavform,self.wfpts = self.get_beat_waveform()  #Beat waveform
		if self.bwaveform is not None:
			self.pqrst_dict = self.pqrst()
		self.plot_wf() #plot for debug

	
	def get_beat_waveform(self,win=[-0.4,0.6]):
		...
		try:
			...
			beat_offset =  beat_offet_org-self.data['start_idx']
			rpk = beat_rpeak_idx-self.data['start_idx']
			bwaveform = self.whol_waveform[beat_onset:beat_offset]
			...
			wfpts = {'beat_onset_org':beat_onset_org,'beat_offset_org':beat_offset_org,
					 'beat_onset':beat_onset,'beat_offset':beat_ofset}
			...
			plt.plot(bwaveform)
			plt.scatter(beat_onset,self.whol_waveform[beat_onset],color='yellow')
	...
	def get_rris(self,in_ms):
		if in_ms:
			rpeaks = [100*item / self.fs for item in self.rpeaks]
		else:
			rpeaks = self.rpeaks
	...
	def diff_pre_avg_nr(self):
		...
		return (self.pre_rri()-self.meanri)/self.rmsrri
	...
	def diff_post_pre_nr_sdrri(self):
		post = self.post_sdrri()
		pre = self.pre_sdrri()
		return (post-pre)/self.absmeansdri
	...
	def plot_wf(self):
		...
		#plt.plot(bwaveform)
		beat_rpeak_idx = self.rpeaks[self.bloc]
		onset = self.wfpts['beat_onset']
		offset = self.wfpts['beat_offset']
		plt.scatter(onset,self.whole_wavform[onset],color='deeppink',marker='>',s=40)
		plt.scatter(offet,self.whole_waveform[offset],color='deeppink',marker='<',s=40)
		rpk = beat_rpeak_idx-self.data['start_idx']
		plt.scatter(rpk,self.whole_waveform[rpk],color='red')
		try:
			p=(self.pqrst_dict['p'][0])+onset
			q=(self.pqrst_dict['q'][0])+onset
			r=(self.pqrst_dict['r'][0])+onset
			s=(self.pqrst_dict['s'][0])+onset
			plt.scatter(p,self.whole_waveform[p],color='cyan')
			plt.scatter(q,self.whole_waveform[q],color='magenta')
			plt.scatter(r,self.whole_waveform[r],color='violet',marker='x')
			plt.scatter(s,self.whole_waveform[s],color='lime')
		except:
			pass
		plt.title(self.label)
		plt.savefig('wvplots/{}_{}.jpg'.format(
								str(self.data['rec_num']),str(self.data['start_idx'])))
		plt.close(fig)
		import time
		time.sleep(.1)
